...
size_t AccountRepository::_add(size_t& clientId, const float& balance, const string& currency, const string& statusStr)
{
    INFO("AccountRepository -> method add (value, status = type::string): called;");

    try
    {
        size_t newAccountId = stoi(Queries::executeCommand(Queries::Accounts::insertAccount(clientId, balance, currency, statusStr)));
...
size_t AccountRepository::add(const Account* account)
{
    INFO("AccountRepository -> method add (obj): called;");

    size_t clientId = account->clientId;
    return add(clientId, account->balance, account->currency, account->status);
}
...
bool AccountRepository::_update(const size_t& accountId, const float& balance, const string& currency, const string& openedDateStr, const string& closedDateStr, const string& status)
{
    INFO("AccountRepository -> method update (value, dates = type::string, clientRegistrationDate = type::string): called;");

    try
    {
        string result = Queries::executeCommand(Queries::Accounts::updateAccount(accountId, balance, currency, openedDateStr, closedDateStr, status));
...
bool AccountRepository::update(const Account* account)
{
    ...
    return update(account->accountId, account->balance, account->currency, account->openedDate, account->closedDate, account->status);
}
